...
async function downloadPlaylist(playListURL: string, outputDir?: string): Promise<string[]> {
  const ids: string[] = [];
  ...
  return new Promise((res, rej) => {
    if (ids.length <= 0) return rej("Invalid url | No videos found.");
...
async function downloadVideos(urls: string[]) {
